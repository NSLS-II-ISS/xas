...
def build_scangroup_interactable(scangroup_node, group_label):
    ...
    scan_labels = [
        html.Div(
            [
                dbc.Checkbox(
                    id={"type": "scan_check", "uid": k, "group": group_label}, style={"display": "inline-block"}
                ),
                html.Div(
                    v.metadata["scan_id"],
...
def build_nested_accordion(
    base_node, groupby_keys: list[str], sort_key: str = None, reverse_order=False, _node_label=""
):
    ...
    if len(next_level_keys) == 0:
        accordion_items = [
            dbc.AccordionItem(
                build_scangroup_interactable(sg_node, group_label=(_node_label + sg_label)), title=sg_label
            )
...
def build_proposal_accordion(proposal_node, groupby_keys, sort_key=None, reverse_order=False):
    proposal_accordion = build_nested_accordion(
        proposal_node, groupby_keys, sort_key=sort_key, reverse_order=reverse_order
    )
...
def build_filter_input(filter_index):
    key_input = dbc.Input(id={"type": "filter_key_input", "index": filter_index}, placeholder="metadata key")
    value_input = dbc.Input(id={"type": "filter_value_input", "index": filter_index}, placeholder="value")
    ...
    [
        ...
        dbc.Row(
            [
                ...
                dbc.Col(dbc.Button("clear figure", id="clear_btn", style={"width": "100%"}), width=4),
            ],
            justify="center",
        ),
    ],
