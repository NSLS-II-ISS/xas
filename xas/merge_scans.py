...
def calc_outliers(df_uid: pd.DataFrame):
    ...
    for sg in df_uid["scan_group"].unique():
        ...
        sg_df_uid = sg_df_uid[
            (sg_df_uid["mut_good"] == True) & (sg_df_uid["muf_good"] == True) & (sg_df_uid["mur_good"] == True)
        ]
        ...
        if sg_datalist:
            try:
                ...
                outlier_df.rename(
                    columns={"mut": "mut_outlier", "muf": "muf_outlier", "mur": "mur_outlier"}, inplace=True
                )
                dev_df = pd.DataFrame(dev_dict, index=sg_df_uid.index)
                dev_df.rename(
                    columns={"mut": "mut_devmean", "muf": "muf_devmean", "mur": "mur_devmean"}, inplace=True
                )
...
def plot_scangroup_outliers(
    scan_group_df: pd.DataFrame, ax_og: plt.Axes, ax_pre: plt.Axes, columns=["mut", "muf", "mur"]
):
...
class ScanGroup:
    def __init__(self, uids, data, metadata) -> None:
        ...
        self.i0_mV = [
            np.array(df["i0"] * 10 ** md_df["ch1_amp_gain"] * 1000) for df, md_df in zip(self.data, self.metadata)
        ]
        self.it_mV = [
            np.array(df["it"] * 10 ** md_df["ch2_amp_gain"] * 1000) for df, md_df in zip(self.data, self.metadata)
        ]
        self.ir_mV = [
            np.array(df["ir"] * 10 ** md_df["ch3_amp_gain"] * 1000) for df, md_df in zip(self.data, self.metadata)
        ]
        self.iff_mV = [
            np.array(df["iff"] * 10 ** md_df["ch4_amp_gain"] * 1000) for df, md_df in zip(self.data, self.metadata)
        ]
    ...
    def current_array_list(self):
        ...
        """
        _current_array_list = [
            np.array(currents) for currents in zip(self.i0_mV, self.it_mV, self.ir_mV, self.iff_mV)
        ]
    ...
    def check_saturation(self):
        ...
        for ind, i_array in enumerate(self.current_array_list):
            if np.any(i_array < -3200):
                print(f"saturated - need to check i0, it, iff for scan: \n {self.uids[ind]}")
    ...
    def check_amplitude(self):
        ...
        for ind, i_array in enumerate(self.current_array_list):
            if np.all(i_array > 0):
                print(f"no useful signal - need to check i0 for scan: \n {self.uids[ind]}")
