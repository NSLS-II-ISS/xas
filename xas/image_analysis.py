...
def determine_beam_position_from_fb_image(
    image, line=420, center_point=655, n_lines=1, truncate_data=True, should_print_diagnostics=True
):
    ...
    sum_lines = sum(
        image[
            :, [i for i in range(int(line - math.floor(n_lines / 2)), int(line + math.ceil(n_lines / 2)))]
        ].transpose()
    )
    if len(sum_lines) > 0:
...
def find_points_moving_with_stage(
    stage_xs, stage_ys, imgs_orig, pix_per_mm=15, pix_per_mm_tol=7, assume_ring_scan=True, plotting=False
):
    ...
    for _img in imgs_orig:
        img = cv2.normalize(
            cv2.cvtColor(_img.astype(np.float32), cv2.COLOR_GRAY2BGR), None, 0, 255, cv2.NORM_MINMAX
        ).astype("uint8")
        imgs.append(img)
    ...
    return {"pix_xy1": pix_xy1, "pix_xy2": pix_xy2, "stage_xy": stage_xy, "timestamp": ttime.time()}
...
class CameraCalibration:
    ...
    def form_pix_subbasis(self, pix_xy, include_offset=False):
        ...
        basis_list += [
            np.vstack([x**i * y ** (j - i) for i in range(j + 1)]).T for j in range(1, self.npoly + 1)
        ]
...
class CameraCalibrationFF:
    ...
    def calibration_data_dict(self):
        return {"pix_xy1": self.pix_xy1, "pix_xy2": self.pix_xy2, "stage_xy": self.stage_xy, "npoly": self.npoly}
