...
class APBBinFileHandler(ISSHandlerBaseShiftRoot):
    ...
    def __init__(self, fpath):
        ...
        for i in range(num_columns - 2):
            derived_data[:, i + 1] = raw_data[
                :, i
            ]  # ((raw_data[:, i] ) - Offsets[i]) / Gains[i]
...
class APBTriggerFileHandler(ISSHandlerBaseShiftRoot):
    ...
    def __init__(self, fpath):
        ...
        derived_data[:, 0] = (
            raw_data[:, 1] + raw_data[:, 2] * 8.0051232 * 1e-9
        )  # Unix timestamp with nanoseconds
    ...
    def _get_dataset(self):
        ...
        for chan in range(1, _num_channels + 1):
            ...
            for key in keys:
                ...
                self._roi_data[(chan, roi_num)] = self._file[
...
class ISSPilatusHDF5Handler(
    Xspress3HDF5Handler, ISSHandlerBaseShiftRoot
):  # Denis: I used Xspress3HDF5Handler as basis since it has all the basic functionality and I more or less understand how it works
    ...
    def _get_dataset(self):
        ...
        _data_columns = [
            self._file[self._key + f"/_{chanroi}Total"][()] for chanroi in self.hdfrois
        ]
...
class ISSPilatusHDF5HandlerLegacy(
    Xspress3HDF5Handler, ISSHandlerBaseShiftRoot
):  # Denis: I used Xspress3HDF5Handler as basis since it has all the basic functionality and I more or less understand how it works
    specs = {"PIL100k_HDF5"} | HandlerBase.specs
    HANDLER_NAME = "PIL100k_HDF5"
    ...
    def __call__(self, *args, frame=None, **kwargs):
        self._get_dataset()
        return_dict = {
            chanroi: self._roi_data[chanroi][frame] for chanroi in self.chanrois
        }
...
def register_all_handlers(db):
    db.reg.register_handler(
        "PIZZABOX_ENC_FILE_TXT_PD", PizzaBoxEncHandlerTxtPD, overwrite=True
    )
    db.reg.register_handler("APB", APBBinFileHandler, overwrite=True)
    db.reg.register_handler("APB_TRIGGER", APBTriggerFileHandler, overwrite=True)
    db.reg.register_handler("PIL100k_HDF5", ISSPilatusHDF5Handler, overwrite=True)
    db.reg.register_handler(
        ISSXspress3HDF5Handler.HANDLER_NAME, ISSXspress3HDF5Handler, overwrite=True
    )
...
def register_all_handlers_legacy(db):
    ...
    db.reg.register_handler("APB", APBBinFileHandler, overwrite=True)
    db.reg.register_handler("APB_TRIGGER", APBTriggerFileHandler, overwrite=True)
    db.reg.register_handler("PIL100k_HDF5", ISSPilatusHDF5HandlerLegacy, overwrite=True)
    db.reg.register_handler(
        ISSXspress3HDF5Handler.HANDLER_NAME, ISSXspress3HDF5Handler, overwrite=True
    )
