...
def xas_energy_grid(
    ...
    after_edge = edge[-1] + get_transition_grid(
        xanes_spacing,
        post_edge[1] - post_edge[0],
        post_edge[0] - edge[-1],
        round_up=True,
    )
    energy_grid = np.unique(
        np.concatenate((preedge, before_edge, edge, after_edge, post_edge))
    )
    energy_grid = energy_grid[
        (energy_grid >= np.min(energy_range)) & (energy_grid <= np.max(energy_range))
    ]
    return energy_grid
...
_GAUSS_SIGMA_FACTOR = 1 / (2 * (2 * np.log(2)) ** 0.5)
...
def bin(
    ...
    if skip_binning:
        ...
        convo_mat = _generate_convolution_bin_matrix(
            binned_energy_grid, interpolated_energy_grid
        )
