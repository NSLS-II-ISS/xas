...
for i in range(2, 201):
    ...
    q_i, sq_i = integrate_pil100k_image_stack(
        ext_data_i["pil100k_image"], df_i["energy"], dist=dist, center_ver=center_ver, center_hor=center_hor
    )
...
def fit_stack_of_patterns(energy, sq, iff, f_real, f_imag):
    ...
    basis = np.vstack((np.ones(energy.size), energy, energy**2, iff, f_lin_use, f_quad_use)).T
...
def kk_calculate_real_from_array(
    ...
    if curve_tolerance is not None:
        output_data = kkcalc.kk.improve_accuracy(
            Full_E, Real_Spectrum, Imaginary_Spectrum, Relativistic_Correction, curve_tolerance, curve_recursion
        )
    else:
