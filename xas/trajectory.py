...
class TrajectoryCreator:
    ...
    def define(
        ...
        if (trajectory_type == "Double Sine/Constant Edge") or (trajectory_type == "standard"):
        ...
        if e is not None:
            try:
                e_bin = xas_energy_grid(
                    e, e0, edge_start, edge_end, preedge_spacing, xanes_spacing, exafs_k_spacing
                )
    ...
    def tile_light(self, reps=1, single_direction=False):
        if not single_direction:
            self.time = np.append(self.time, (self.time + self.time[-1] + self.time[1] - self.time[0]))
        ...
        self.time = np.hstack([i * (self.time[-1] + self.time[1] - self.time[0]) + self.time for i in range(reps)])
    ...
    def tile(self, reps=1, single_direction=False):
        if not single_direction:
            self.time_grid = np.append(
                self.time_grid, (self.time_grid + self.time_grid[-1] + self.time_grid[1] - self.time_grid[0])
            )
        ...
        self.time_grid = np.hstack(
            [
                i * (self.time_grid[-1] + self.time_grid[1] - self.time_grid[0]) + self.time_grid
                for i in range(reps)
            ]
        )
