...
_LABEL_DICT = {"mu": "mu", "normalized": "mu norm", "flattened": "mu flat"}
...
class DataManager:
    ...
    def _process_data(self, uid, channel, params=None):
        ...
        if data_type.lower() in self.data_types:
            if data_type.lower() == "xas":
                ...
                (
                    processed_result["norm"],
                    pre_edge_curve,
                    post_edge_curve,
                    norm_parameters,
                ) = LarchCalculator.normalize(
                    energy, mu_in, flatten_output=False, return_norm_parameters=True, **params
                )
                ...
                processed_result["k"], processed_result["chi"], autobk_params = LarchCalculator.auto_background(
                    energy, mu_in, return_autobk_params=True, **params
                )
    ...
    def get_processed_data(self, uid, channel, processing_parameters=None):
        if uid in self.processed_data.keys():
            ...
            if channel not in self.processed_data[
                uid
            ].keys() or processing_parameters != self.get_processing_parameters(uid, channel):
    ...
    def create_user_group_in_metadata(self, uids: list[str], group_name: str, channels_of_interest: list[str]):
        for uid in uids:
...
def min_value_for_metadata_key(node: Node, key):
    ...
    return min(
        [client_obj.metadata[key] for client_obj in node.values_indexer if key in client_obj.metadata.keys()],
        default=None,
    )
...
def sort_nodes_by_metadata_key(list_of_nodes: list[Node], sort_key, node_labels: list = None, reverse_order=False):
    ...
    if node_labels is None:
        ...
        return sorted(
            list_of_nodes,
            key=lambda node: (
                min_value_for_metadata_key(node, sort_key) is None,
                min_value_for_metadata_key(node, sort_key),
            ),
            reverse=reverse_order,
        )
    else:
        sorted_nodes_and_labels = sorted(
            zip(list_of_nodes, node_labels),
            key=lambda node: (
                min_value_for_metadata_key(node[0], sort_key) is None,
                min_value_for_metadata_key(node[0], sort_key),
            ),
            reverse=reverse_order,
        )
