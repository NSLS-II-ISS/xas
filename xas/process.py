...
def process_interpolate_bin(
    doc, db, draw_func_interp=None, draw_func_bin=None, cloud_dispatcher=None, print_func=None, dump_to_tiff=False
):
    # logger = get_logger()
...
def process_interpolate_bin_from_uid(
    ...
    logger = get_logger()
    hdr, primary_df, extended_data, comments, path_to_file, file_list, data_kind = get_processed_df_from_uid(
        uid,
        db,
        logger=logger,
        draw_func_interp=draw_func_interp,
            ...
            for stream_name in stream_names:
                if stream_name == "pil100k_stream":
                    apb_trigger_pil100k_timestamps = load_apb_trig_dataset_from_db(
                        db, uid, use_fall=True, stream_name="apb_trigger_pil100k"
                    )
    ...
    if "spectrometer" in hdr.start.keys():
        if hdr.start["spectrometer"] == "von_hamos":
            extended_data, comments, file_paths = process_von_hamos_scan(
                primary_df, extended_data, comments, hdr, path_to_file, db=db
            )
            data_kind = "von_hamos"
