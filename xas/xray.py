...
def encoder2energy(encoder, pulses_per_deg, offset=0):
    ...
    """
    return -12398.42 / (
        2 * 3.1356 * np.sin(np.deg2rad((encoder / pulses_per_deg) - float(offset)))
    )
...
def energy2encoder(energy, pulses_per_deg, offset=0):
    ...
    "ASIN(B/A)*180/C - E")
    """
    return pulses_per_deg * (
        np.degrees(np.arcsin(-12398.42 / (2 * 3.1356 * energy))) - float(offset)
    )
...
def generate_energy_grid_from_dict(scan_parameters):
    if "grid_kind" in scan_parameters.keys():
        ...
        grid_kind = "xas"
    if grid_kind == "xas":
        energy, time_grid = generate_energy_grid(
            scan_parameters["e0"],
        ...
        energy, time_grid = generate_linear_energy_grid(
            scan_parameters["energy_min"],
...
def generate_emission_energy_grid(
    e0,
    preline_start,
    mainline_start,
    mainline_end,
    ...
    energy_grid_preline = np.arange(
        e0 + preline_start, e0 + mainline_start, preline_stepsize
    )
    time_grid_preline = np.ones(energy_grid_preline.size) * preline_dwelltime

    energy_grid_mainline = np.arange(
        e0 + mainline_start, e0 + mainline_end, mainline_stepsize
    )
    time_grid_mainline = np.ones(energy_grid_mainline.size) * mainline_dwelltime

    energy_grid_postline = np.arange(
        e0 + mainline_end, e0 + postline_end + postline_stepsize, postline_stepsize
    )
    time_grid_postline = np.ones(energy_grid_postline.size) * postline_dwelltime

    energy_grid = np.hstack(
        (energy_grid_preline, energy_grid_mainline, energy_grid_postline)
    )
...
def generate_emission_energy_grid_from_dict(scan_parameters):
    ...
    energy_grid, time_grid = generate_emission_energy_grid(
        scan_parameters["e0"],
