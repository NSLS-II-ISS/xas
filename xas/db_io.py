...
def load_apb_dataset_from_db(db, uid):
    ...
    apb_dataset = pd.DataFrame(
        apb_dataset, columns=["timestamp", "i0", "it", "ir", "iff", "aux1", "aux2", "aux3", "aux4"]
    )
    ...
    energy_dataset = pd.DataFrame(energy_dataset, columns=["ts_s", "ts_ns", "encoder", "index", "state"])
    ...
    if input_type == "tiff":
        ...
        for j in range(4):
            ...
            spectra[f"pil100k_ROI{j+1}"] = pd.DataFrame(
                np.vstack((pil100k_timestamps, this_spectrum)).T, columns=["timestamp", f"pil100k_ROI{j+1}"]
            )
...
def get_uid_tree_for_proposal(db, year, cycle, proposal):
    ...
    for i, row in uid_df.iterrows():
        if not row["grouped"]:
            if (row["scan_group_uid"] is None) or ((uid_df["scan_group_uid"] == row["scan_group_uid"]).sum() == 1):
