...
from xas.fitting import (
    get_normalized_gaussian_scan,
    estimate_center_and_width_of_peak,
    fit_gaussian,
...
def compute_rowland_circle_geometry(x_src, y_src, R, bragg_deg, det_dR):
    ...
    x_det = +2 * x_cr * np.cos(bragg) * np.cos(bragg) - det_dR * np.cos(
        np.pi - 2 * bragg
    )
    y_det = -2 * x_cr * np.cos(bragg) * np.sin(bragg) - det_dR * np.sin(
        np.pi - 2 * bragg
    )
...
def _rotate_xyz(omega, bragg, xyz):
    rmat = Rotation.from_euler(
        "zyz", [-(90 - bragg), omega, (90 - bragg)], degrees=True
    ).as_matrix()
    return rmat @ xyz
...
def _compute_rotated_rowland_circle_geometry(
    x_cr_main, y_cr_main, x_det, y_det, bragg_deg, dz
):
    ...
    roll_cr_rot = (
        90 - np.rad2deg(np.arctan(np.abs((y_det - y_cr_rot) / (x_det - x_cr_rot)))) / 2
    )
...
def compute_rotated_rowland_circle_geometry(x_src, y_src, R, bragg, dz):
    x_cr_main, y_cr_main, x_det, y_det = compute_rowland_circle_geometry(
        x_src, y_src, R, bragg, 0
    )
    (
        x_cr_rot,
        y_cr_rot,
        roll_cr_rot,
        yaw_cr_rot,
    ) = _compute_rotated_rowland_circle_geometry(
        x_cr_main, y_cr_main, x_det, y_det, bragg, dz
    )
    return x_cr_rot, y_cr_rot, roll_cr_rot, yaw_cr_rot
...
_pilatus_roi_colors = {1: "tab:blue", 2: "tab:orange", 3: "tab:green"}
...
def analyze_elastic_fly_scan(db, uid, rois=None, plot_func=None):
    ...
    for i in rois:
        ...
        Ecen, fwhm, intensity_cor, intensity_fit, intensity_fit_raw = fit_gaussian(
            energy, intensity, Ecen0, fwhm0
        )
...
def parse_rixs_scan(db, file_with_uids, xes_normalization=True):
    energies_out, uids_herfd, uids_xes = parse_file_with_uids(
        file_with_uids, xes_normalization=xes_normalization
    )
    xes_data = []
    herfd_data = []
...
def convert_rixs_to_energy_transfer(Ein, Eout, herfd):
    ...
    transfer_step = np.min(
        [np.min(np.abs(np.diff(np.unique(Ein)))), np.abs(Eout[1] - Eout[0])]
    ) * np.sqrt(2)
    ...
    for idx in range(Ein.size):
        ...
        rixs[idx, :] = np.interp(
            dE, each_transfer[idx_ord], herfd[idx_ord, idx], left=0, right=0
        )
...
files = (
    "/nsls2/xf08id/users/2021/1/300001/Co3O4 RIXS 0001.dat",
