...
def _compute_rotated_rowland_circle_geometry(
    x_cr_main, y_cr_main, x_det, y_det, bragg_deg, dz, output_omega=False
):
...
def compute_rotated_rowland_circle_geometry(x_src, y_src, R, bragg, dz, output_omega=False):
    ...
    output = _compute_rotated_rowland_circle_geometry(
        x_cr_main, y_cr_main, x_det, y_det, bragg, dz, output_omega=output_omega
    )
    return output
...
def pilatus_position_roi(image, dx, dy):
    ...
    com_x, com_y = pilatus_image_com(
        image, (int(com_x0 - dx / 2 * 1.5), int(com_y0 - dy / 2 * 1.5), int(dx * 1.5), int(dy * 1.5))
    )
    print(int(com_x0 - dx / 2 * 1.5), int(com_y0 - dy / 2 * 1.5), int(dx * 1.5), int(dy * 1.5))
    return com_x, com_y
