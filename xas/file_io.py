...
def load_dataset_from_files(db, uid):
    def load_adc_trace(filename=""):
        ...
        if os.path.isfile(filename):
            ...
            df["adc"] = (
                df_raw["adc"].apply(
                    lambda x: (int(x, 16) >> 8) - 0x40000 if (int(x, 16) >> 8) > 0x1FFFF else int(x, 16) >> 8
                )
                * 7.62939453125e-05
            )
            return df
    ...
    def load_trig_trace(filename=""):
        keys = ["times", "timens", "encoder", "counter", "di"]
        if os.path.isfile(filename):
            df = pd.read_table(filename, delim_whitespace=True, comment="#", names=keys, index_col=False)
...
def save_interpolated_df_as_file(path_to_file, df_orig, comments):
    ...
    df = df[cols]
    np.savetxt(path_to_file, df.values, fmt=fmt, delimiter=" ", header=header, comments=comments)
...
def save_binned_df_as_file(path_to_file, df_orig, comments):
    ...
    df = df[cols]
    np.savetxt(path_to_file, df.values, fmt=fmt, delimiter=" ", header=f"# {header}", comments=comments)
...
xs_channel_comb_dict = {
    ...
    "xs_roi02": ["xs_ch01_roi02", "xs_ch02_roi02", "xs_ch03_roi02", "xs_ch04_roi02"],
    "xs_roi03": ["xs_ch01_roi03", "xs_ch02_roi03", "xs_ch03_roi03", "xs_ch04_roi03"],
    "xs_roi04": ["xs_ch01_roi04", "xs_ch02_roi04", "xs_ch03_roi04", "xs_ch04_roi04"],
}


pil100k_channel_list = ["pil100k_roi1", "pil100k_roi2", "pil100k_roi3", "pil100k_roi4"]
pil100k_channel_comb_dict = {
    "xes": ["pil100k_roi1", "pil100k_roi2"],
}
