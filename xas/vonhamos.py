...
def apply_calibration_for_roi(
    df, extended_data, md, uid_calibration, db, roi="roi1", roi_dict=None, detector="Pilatus 100k", droi=5
):
    if roi_dict is None:
        roi_dict = md["detectors"][detector]["config"]["roi"]
    ...
    if (
        ...
        and (not scan_and_calibration_roi_match(md, uid_calibration, db, roi=roi, detector=detector))
        and (not enforce_roi)
    ):
        do_trivial_calibration = True
    ...
    if do_trivial_calibration:
        ...
        p_xy, p_xe = process_calibration_for_roi_uid(
            uid_calibration, db, roi=roi, roi_dict=roi_dict_calibration, detector=detector
        )
...
def process_von_hamos_scan_legacy(
    df, extended_data, comments, hdr, path_to_file, detector="Pilatus 100k", roi="auto", droi=5, save_dat=True
):
