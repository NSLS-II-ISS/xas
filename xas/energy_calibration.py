...
def compute_shift_between_spectra(energy, mu, energy_ref_roi, mu_ref_roi):
    energy_ref_roi_norm = (energy_ref_roi - energy_ref_roi.min()) / (
        energy_ref_roi.max() - energy_ref_roi.min()
    )
    ...
    def interpolated_spectrum(pars):
        ...
        basis = np.vstack(
            (
                ...
                x,
                energy_ref_roi_norm,
                energy_ref_roi_norm**2,
                energy_ref_roi_norm**3,
            )
        ).T
...
def compute_energy_shift_and_broadening_between_spectra(
    energy, mu, energy_ref, mu_ref, e0=8333, de=200
):
    ...
    energy_roi_norm = (energy_roi - energy_roi.min()) / (
        energy_roi.max() - energy_roi.min()
    )

    # shift_guess = compute_shift_between_spectra_alt(energy_ref, mu_ref, energy_roi, mu_roi, e0=e0, de=de)[0]
    shift_guess = compute_shift_between_spectra(energy_ref, mu_ref, energy_roi, mu_roi)[
        0
    ]
    print(shift_guess)
    ...
    def get_mu_fit(pars):
        ...
        conv_matrix = gaussian_conv_matrix(
            fine_grid_energy_ref - shift, energy_roi, sigma=sigma
        )
        mu_ref_conv = conv_matrix @ fine_grid_mu_ref

        basis = np.vstack((mu_ref_conv, np.ones(energy_roi.shape), energy_roi_norm)).T
...
def get_energy_offset(
    uid, db, db_proc, dE=25, plot_fun=None, attempts=5, sleep_time=1, full_return=False
):
    start = db[uid].start
    ...
    if fname_raw.endswith(".raw"):
        ...
        try:
            ...
            shift, mu_fit = compute_shift_between_spectra(
                energy, mu, energy_ref_roi, mu_ref_roi
            )
            e_cor = e0 + shift
...
def get_energy_offset_for_proposal(db, year, cycle, proposal, db_proc, dE=25):
    ...
    for i, uid in enumerate(uids):
        ...
        if ("exit_status" in hdr.stop.keys()) and (hdr.stop["exit_status"]):
            _e_ref, _e_obs, _, mu_ref, mu_fit = get_energy_offset(
                uid, db, db_proc, dE=dE, attempts=1, sleep_time=0, full_return=True
            )
            if _e_obs:
...
for element in iss_foils:
    for edge in xraydb.xray_edges(element):
        ...
        if (energy >= 4500) and (energy <= 32000):
            _iss_foil_data.append({"element": element, "edge": edge, "energy": energy})
...
def find_correct_foil(energy=None, element="Cu", edge="K"):
    ...
    foils_options = iss_foils_df.loc[
        (iss_foils_df["energy"] > energy - 200)
    ...
    indx = foils_options.index[
        (foils_options["element"] == element) & (foils_options["edge"] == edge)
    ]
